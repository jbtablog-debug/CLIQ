...
`;
            ...
            // Only add overlay logic if not disabled
            if (!isDisabled) {
      ...
      <div class="milktea-overlay">
        <div class="milktea-content">
          ...
          <p style="text-align:center;">
            Price: <span class="milktea-price" id="milktea-price-${uniqueId}">P${basePrice.toFixed(2)}</span>
          </p>
    ...
    function updateOverlayPrice() {
        ...
        let base = Array.isArray(milktea.sizes) && milktea.sizes.length > 0
            ? parseFloat(milktea.sizes[idx].price) || 0
            : parseFloat(milktea.price) || 0;
    ...
    overlayAddToCartBtn.onclick = function () {
        ...
        const selectedSize = (Array.isArray(milktea.sizes) && milktea.sizes.length > 0)
            ? milktea.sizes[selectedSizeIdx].size
            : "Regular";
        const quantity = overlayQty ? parseInt(overlayQty.value) : 1;
        let base = Array.isArray(milktea.sizes) && milktea.sizes.length > 0
            ? parseFloat(milktea.sizes[selectedSizeIdx].price) || 0
            : parseFloat(milktea.price) || 0;
