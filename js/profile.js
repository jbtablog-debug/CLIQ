...
// --- Fetch & Display Profile Info ---
function fetchAndDisplayProfile() {
  const username = (localStorage.getItem("loggedInUsername") || "").toLowerCase();
  if (!username) return;
      ...
      if (snapshot.exists()) {
        ...
        for (const uid in users) {
          ...
          if ((user.username || "").toLowerCase() === username) {
            // If user is admin, do not display profile and redirect or show message
            if (user.role === "admin") {
              document.querySelector(".profile-container").innerHTML = "<p style='color:#d9534f;text-align:center;font-weight:bold;'>Admin accounts cannot view this profile page.</p>";
        ...
        if (userData) {
          ...
          // Store userId for username change
